...
case 'delete':
	...
	$sql = "SELECT imagem, titulo FROM slide WHERE id = $id;";
	...
	insert_log("removeu o slide <b>$row[1]</b>");
	header("Location: slides.php?alerta=removido&slide=$row[1]");
	break;
...
case 'update':
	...
	$sql = "UPDATE slide SET titulo = '$titulo', descricao = '$descricao', imagem = '$img_url' WHERE id = $id;";
	$conn->query($sql);

	insert_log("atualizou o slide <b>$row[1]</b>");
	header("Location: slides.php?alerta=atualizado&slide=$row[1]");
	break;
...
case 'insert':
	...
	$sql = "INSERT INTO slide VALUES(NULL, '$titulo', '$descricao', '$img_url');";
	$conn->query($sql);

	insert_log("inseriu o slide <b>$titulo</b>");
	header("Location: slides.php?alerta=adicionado&slide=$titulo");
	break;

case 'editar':
	$sql = "SELECT titulo, descricao, imagem FROM slide WHERE id = $id";
	$query = $conn->query($sql);
	$row = $query->fetch_array();

	do_header("Editar slide ($row[0])", "slides", 0);
...
<form action="slides.php?a=update<?php echo "&id=$id"; ?>" method="POST" class="col-8" enctype="multipart/form-data">
	<p>
		<label>Título:</label>
	...
	<p><input type="submit" class="btn btn-primary btn-sm" value="Atualizar slide"></p>
</form>
...
case 'adicionar':
	do_header("Adicionar slide", "slides", 2);
...
<form action="slides.php?a=insert" method="POST" class="col-8" enctype="multipart/form-data">
	<p>
		<label>Título:</label>
	...
	<p><input type="submit" class="btn btn-primary btn-sm" value="Adicionar novo slide"></p>
</form>
...
default:
	...
	do_header("Controle de slides", "slides", 1);
...
if (isset($alert))
{
	...
	{
		case 'removido':
			do_alert("alert-success", "Slide <b>$slide</b> deletado!");
			break;

		case 'atualizado':
			do_alert("alert-success", "Slide <b>$slide</b> atualizado!");
			break;

		case 'adicionado':
			do_alert("alert-success", "Slide <b>$slide</b> adicionado!");
			break;
	}
}
...
<section class="row text-center placeholders justify-content-center">
	...
	$sql = "SELECT * FROM slide ORDER BY titulo ASC LIMIT $min, $max;";
	$query = $conn->query($sql);
	...
	while ($row = $query->fetch_array())
	{
		echo "<div class=\"col-7 col-sm-4 mb-3 mb-sm-5 placeholder\">
			<div class=\"actions text-center text-md-right\">
				<a href=\"slides.php?a=editar&id=$row[0]\"><span class=\"oi oi-align-center\" data-glyph=\"pencil\"></span></a>";

				if ($access) echo "<a href=\"slides.php?a=delete&id=$row[0]\"><span class=\"oi oi-align-center\" data-glyph=\"x\"></span></a>";
...
pagination($page, $pages, "admin/slides.php?");
